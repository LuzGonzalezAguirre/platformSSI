...
export const ProblemWizardPage: React.FC = () => {
  ...
  const canSubmit = useMemo(() => Object.values(stepValidation).every((value) => value.valid), [stepValidation]);
  ...
  const handleSave = async () => {
    try {
      if (isEditMode && id) {
        ...
        showNotification('success', 'Problem created', 'The new 8D problem was created successfully.');
        navigate(`/quality/problems/${newProblem.id}/edit`, { replace: true });
      }
    } catch (error: any) {
      showNotification('error', 'Save failed', error.response?.data?.detail || error.message || 'Unable to save the problem.');
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      showNotification('error', 'Submit failed', error.response?.data?.detail || error.message || 'Unable to submit the problem.');
    }
  };
  ...
  if (isEditMode && isLoadingProblem) {
    return <div style={styles.loadingContainer}><p>Loading problem...</p></div>;
  }

  return (
    <div style={styles.container}>
      {notification && (
        <div role="status" style={{ ...styles.notification, ...(notification.type === 'success' ? styles.notificationSuccess : notification.type === 'error' ? styles.notificationError : styles.notificationInfo) }}>
          <div style={{ ...styles.notificationIcon, ...(notification.type === 'success' ? styles.notificationIconSuccess : notification.type === 'error' ? styles.notificationIconError : styles.notificationIconInfo) }}>
          ...
          <button type="button" onClick={() => setNotification(null)} style={styles.notificationClose} aria-label="Close notification">×</button>
        </div>
      )}
      ...
      <div style={styles.header}>
        <div>
          ...
          <h1 style={styles.title}>{isEditMode ? `Edit Problem ${problem?.problem_number || `#${id}`}` : 'Create New Problem'}</h1>
          <p style={styles.subtitle}>Complete the 8D problem solving methodology step by step</p>
        </div>
        {problem?.status && <div style={styles.statusBadge}>Status: {problem.status_display}</div>}
      </div>
...
const styles: { [key: string]: React.CSSProperties } = {
  ...
  loadingContainer: { padding: '2rem', textAlign: 'center', color: 'var(--color-text-secondary)' },
  notification: { position: 'fixed', top: '1.25rem', right: '1.25rem', zIndex: 9999, minWidth: '320px', maxWidth: '430px', display: 'flex', alignItems: 'flex-start', gap: '0.75rem', padding: '0.9rem 1rem', borderRadius: '0.65rem', border: '1px solid var(--color-border)', boxShadow: '0 12px 28px rgba(15, 23, 42, 0.18)', backgroundColor: 'var(--color-bg-secondary)' },
  notificationSuccess: { borderLeft: '4px solid #16a34a' },
  notificationError: { borderLeft: '4px solid #dc2626' },
  notificationInfo: { borderLeft: '4px solid #2563eb' },
  notificationIcon: { width: '1.75rem', height: '1.75rem', borderRadius: '999px', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.85rem', fontWeight: 800, flexShrink: 0 },
  notificationIconSuccess: { backgroundColor: '#dcfce7', color: '#15803d' },
  notificationIconError: { backgroundColor: '#fee2e2', color: '#b91c1c' },
  notificationIconInfo: { backgroundColor: '#dbeafe', color: '#1d4ed8' },
  notificationText: { flex: 1, minWidth: 0 },
  notificationTitle: { color: 'var(--color-text-primary)', fontSize: '0.875rem', fontWeight: 700, marginBottom: '0.15rem' },
  notificationMessage: { color: 'var(--color-text-secondary)', fontSize: '0.78rem', lineHeight: 1.4, overflowWrap: 'anywhere' },
  notificationClose: { border: 'none', backgroundColor: 'transparent', color: 'var(--color-text-secondary)', cursor: 'pointer', fontSize: '1.2rem', lineHeight: 1, padding: 0 },
  header: { display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1.5rem' },
  backButton: { display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.5rem', backgroundColor: 'transparent', color: '#3b82f6', border: 'none', fontSize: '0.875rem', fontWeight: 500, cursor: 'pointer', marginBottom: '0.5rem' },
  backIcon: { width: '1.25rem', height: '1.25rem' },
  title: { fontSize: '1.875rem', fontWeight: 700, color: 'var(--color-text-primary)', marginBottom: '0.25rem' },
  subtitle: { color: 'var(--color-text-secondary)', fontSize: '0.875rem' },
  statusBadge: { padding: '0.5rem 1rem', backgroundColor: '#dbeafe', color: '#1e40af', borderRadius: '0.375rem', fontSize: '0.875rem', fontWeight: 500 },
};
