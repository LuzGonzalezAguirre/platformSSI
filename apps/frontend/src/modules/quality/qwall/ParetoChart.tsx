...
export default function ParetoChart({ startDate, endDate, includeTest, buId, lang = "es" }: ParetoChartProps) {
  ...
  const H = isFullscreen ? 390 : 240;
  const padL = 34;
  const padR = 40;
  const padT = 18;
  const padB = isFullscreen ? 82 : 72;
  ...
  const maxLabel = isFullscreen ? 28 : 18;
  ...
  const svgContent = (
    <svg width="100%" viewBox={`0 0 ${W} ${H}`} style={{ overflow: "visible" }}>
      ...
      {items.map((it, i) => {
        ...
        const label = compactLabel(it.name || it.code, maxLabel);
        return (
          <g
            key={it.code}
          ...
          >
            ...
            <text
              ...
              y={padT + chartH + 15}
              textAnchor="end"
              transform={`rotate(-32 ${barCenterX(i)} ${padT + chartH + 15})`}
              fontSize={isFullscreen ? 8.5 : 7.5}
              fontWeight={600}
  ...
  const tooltipEl = hovered ? (
    <div style={{
      ...
      <div style={{ fontWeight: 700, marginBottom: "0.25rem", whiteSpace: "normal" }}>{hovered.name || hovered.code}</div>
  ...
  if (isFullscreen) {
    return (
      <div style={{ position: "fixed", inset: 0, zIndex: 500, background: "var(--color-bg)", display: "flex", flexDirection: "column", padding: "1.5rem" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1rem" }}>
          ...
          <div style={{ display: "flex", gap: "0.4rem" }}>
            <GranularityToggle value={granularity} onChange={setGranularity} lang={lang} />
            <button onClick={() => setIsFullscreen(false)} style={{ ...btnBase, background: "var(--color-surface)", color: "var(--color-text-secondary)" }}>
              {l ? "✕ Cerrar" : "✕ Close"}
            </button>
          </div>
        </div>
