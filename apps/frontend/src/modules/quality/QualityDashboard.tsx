...
interface PartRow {
  part_no: string; workcenter: string; bu: string;
...
// ── Trend Chart (SVG simple) ──────────────────────────────────────────────────

function TrendChart({ data, lang }: { data: TrendRow[]; lang: "es" | "en" }) {
  if (!data.length) return <div style={{ color: "var(--color-text-secondary)", fontSize: "0.8125rem" }}>Sin datos</div>;

  const W = 500; const H = 100;
  const max = Math.max(...data.map((d) => d.total_qty), 1);
  const pts = data.map((d, i) => {
    const x = (i / Math.max(data.length - 1, 1)) * W;
    const y = H - (d.total_qty / max) * H;
    return `${x},${y}`;
  });
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", marginTop: "4px" }}>
        {labels.map((d) => (
          <span key={d.date} style={{ fontSize: "10px", color: "var(--color-text-tertiary)" }}>
            {d.date.slice(5)}
          </span>
        ))}
      </div>
      <div style={{ display: "flex", gap: "1rem", marginTop: "0.5rem" }}>
        <span style={{ display: "flex", alignItems: "center", gap: "4px", fontSize: "11px", color: "var(--color-text-secondary)" }}>
          <span style={{ width: "16px", height: "2px", background: "#3b82f6", display: "inline-block", borderRadius: "1px" }} />
          {lang === "es" ? "Qty diaria" : "Daily qty"}
        </span>
      </div>
    </div>
  );
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
      <div style={{ display: "flex", gap: "6px", marginBottom: "2px" }}>
        <span style={{ width: "130px", fontSize: "10px", color: "var(--color-text-tertiary)" }}>Defecto</span>
        <span style={{ flex: 1, fontSize: "10px", color: "var(--color-text-tertiary)" }}>Cantidad</span>
        <span style={{ width: "32px", fontSize: "10px", color: "var(--color-text-tertiary)", textAlign: "right" }}>pcs</span>
        <span style={{ width: "36px", fontSize: "10px", color: "var(--color-text-tertiary)", textAlign: "right" }}>acum</span>
      </div>
      {rows.map((r) => {
        const cumColor = r.cumulative_pct <= 50 ? "#ef4444" : r.cumulative_pct <= 80 ? "#f59e0b" : "#10b981";
        return (
          <div key={r.scrap_reason} style={{ display: "flex", alignItems: "center", gap: "6px" }}>
            <span style={{ width: "130px", fontSize: "11px", color: "var(--color-text-secondary)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
              {r.scrap_reason}
            </span>
            <div style={{ flex: 1, position: "relative", height: "16px", background: "var(--color-border)", borderRadius: "3px" }}>
              <div style={{ ...s.barFill, width: `${(r.total_qty / maxQty) * 100}%`, background: "#3b82f6" }} />
              <div style={{ position: "absolute", top: 0, bottom: 0, left: "80%", width: "1.5px", background: "#f59e0b", opacity: 0.8 }} />
            </div>
            <span style={{ width: "32px", fontSize: "11px", color: "var(--color-text-primary)", textAlign: "right" }}>{r.total_qty}</span>
            <span style={{ width: "36px", fontSize: "11px", color: cumColor, textAlign: "right", fontWeight: 600 }}>{r.cumulative_pct.toFixed(0)}%</span>
          </div>
        );
      })}
      <div style={{ marginTop: "4px", fontSize: "10px", color: "var(--color-text-tertiary)" }}>
        línea amarilla = umbral 80/20
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "7px" }}>
      {rows.map((r) => {
        ...
        return (
          <div key={`${r.part_no}|${r.workcenter}`} style={{ display: "flex", alignItems: "center", gap: "8px" }}>
            <span style={{ width: "50px", fontSize: "11px", color: "var(--color-text-secondary)", fontWeight: 600 }}>{r.part_no}</span>
            <div style={{ flex: 1, position: "relative", height: "16px", background: "var(--color-border)", borderRadius: "3px", overflow: "hidden" }}>
              <div style={{ ...s.barFill, width: `${(r.scrap_qty / maxQty) * 100}%`, background: barColor, opacity: 0.8 }} />
            </div>
            <span style={{ width: "38px", fontSize: "11px", color: "var(--color-text-primary)", textAlign: "right" }}>{r.scrap_qty}</span>
      ...
      <div style={{ fontSize: "10px", color: "var(--color-text-tertiary)", marginTop: "2px" }}>
        color rojo/ámbar = costo alto relativo a cantidad
      </div>
    </div>
  );
}
      ...
      <table style={{ width: "100%", borderCollapse: "collapse" as const, fontSize: "0.8125rem" }}>
        <thead>
          <tr style={{ borderBottom: "1px solid var(--color-border)" }}>
            {["BU", l ? "Workcenter" : "Workcenter", l ? "Producción" : "Production", "Scrap", "Yield", l ? "Turno A" : "Shift A", l ? "Turno B" : "Shift B", l ? "Costo" : "Cost"].map((h) => (
      ...
      <div style={s.modal}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "8px", marginBottom: "1.25rem" }}>
          <div style={{ display: "grid", gridTemplateColumns: "80px 1fr 1fr 80px 80px 32px", gap: "6px", marginBottom: "4px" }}>
            {[l ? "Nivel" : "Level", l ? "BU" : "BU", l ? "Workcenter" : "Workcenter", "Yield ≥", "Scrap ≤", ""].map((h) => (
        ...
        <>
          ...
          <div style={s.row2}>
            <div style={s.card}>
              ...
              <TrendChart data={data?.trend ?? []} lang={lang} />
            </div>
            <div style={s.card}>
