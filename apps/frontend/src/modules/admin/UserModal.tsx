...
export default function UserModal({ isOpen, onClose, onSubmit, user, roles, mode }: UserModalProps) {
  ...
  useEffect(() => {
    if (!isOpen) return;
    if (mode === "create") {
      setForm({ employee_id: "", first_name: "", last_name: "", email: "", plant: "", job_title: "", preferred_language: "es", password: "" });
      setSelectedRoles([]);
    } else if (mode === "edit" && user) {
      setForm({ first_name: user.first_name, last_name: user.last_name, email: user.email, plant: user.plant, job_title: user.job_title || "", preferred_language: user.preferred_language });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (mode !== "reset-password" && selectedRoles.length === 0) {
      setError(t("users.modal.rolesRequired"));
      return;
    }
    setLoading(true);
    setError(null);
  ...
  return (
    <div style={s.overlay}>
      <div style={s.modal}>
        <div style={s.header}>
          <h2 style={s.title}>
            {mode === "create" ? t("users.modal.createTitle") :
             mode === "edit" ? t("users.modal.editTitle") :
             t("users.modal.resetPasswordTitle")}
          </h2>
        ...
        <form onSubmit={handleSubmit} style={s.form}>
          {mode !== "reset-password" && (
            <>
              {mode === "create" && (
                <div style={s.field}>
                  <label style={s.label}>{t("users.modal.employeeId")} *</label>
                  <input
                    style={s.input}
              ...
              <div style={s.row}>
                <div style={s.field}>
                  <label style={s.label}>{t("users.modal.jobTitle")}</label>
                  <input
                    style={s.input}
                    value={form.job_title || ""}
                    onChange={(e) => setForm({ ...form, job_title: e.target.value })}
                    placeholder={t("users.modal.jobTitlePlaceholder")}
                  />
                </div>
                <div style={s.field}>
                  <label style={s.label}>{t("users.modal.plant")}</label>
                  <input
                    style={s.input}
                    value={form.plant || ""}
              ...
              <div style={s.field}>
                <label style={s.label}>
                  ...
                  <span style={s.labelHint}>
                    {selectedRoles.length} {t("users.modal.rolesSelected", { count: selectedRoles.length })}
                  </span>
                </label>
                <div style={s.rolesBox}>
                  {systemRoles.length > 0 && (
                    <div style={s.roleGroup}>
                      <span style={s.roleGroupLabel}>{t("users.modal.systemRoles")}</span>
                        ...
                        {systemRoles.map((r) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <span>{t(`roles.${r.value}`, { defaultValue: r.label })}</span>
                  ...
                  {customRoles.length > 0 && (
                    <div style={s.roleGroup}>
                      <span style={s.roleGroupLabel}>{t("users.modal.customRoles")}</span>
                      <div style={s.roleGrid}>
                        {customRoles.map((r) => {
                          const active = selectedRoles.includes(r.value);
                          return (
              ...
              <div style={s.row}>
                <div style={s.field}>
                  <label style={s.label}>{t("users.modal.language")}</label>
                ...
                <div style={s.field}>
                  <label style={s.label}>{t("users.modal.email")}</label>
              ...
              {mode === "create" && (
                <div style={s.field}>
                  <label style={s.label}>{t("users.modal.password")} *</label>
          ...
          {mode === "reset-password" && (
            <>
              <div style={s.field}>
                <label style={s.label}>{t("users.modal.newPassword")} *</label>
              ...
              <div style={s.field}>
                <label style={s.label}>{t("users.modal.confirmPassword")} *</label>
