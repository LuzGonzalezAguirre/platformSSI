...
import DowntimeStackedChart from "../overview/DowntimeStackedChart";
import { MaintenanceService } from "../overview/overview.service";
import { DowntimeByMonth } from "../overview/types";
...
function WRAgeChart({ rows, lang }: {
  ...
  return (
    <div style={card}>
      ...
      <DonutChart items={items} getLabel={(lbl) => lbl} getColor={(_, i) => COLORS[i % COLORS.length]} lang={lang} />
    </div>
  );
}

function TopFailureChart({ byFailure, lang }: { byFailure: GroupedItem[]; lang: string }) {
  const l = lang === "es";
  const items = byFailure.slice(0, 8);
  ...
  const colW = items.length ? chartW / items.length : chartW;
          ...
          {[0.25, 0.5, 0.75, 1].map((pct) => {
            ...
            return (
              <g key={pct}>
                ...
                <text x={padL - 4} y={y + 3} textAnchor="end" fontSize={7} fill="var(--color-text-secondary)">{(maxH * pct).toFixed(0)}h</text>
              </g>
            );
          })}
          {items.map((item, i) => {
            ...
            const lbl = item.label && item.label !== "—" ? item.label : "Unknown";
            ...
            return (
              <g key={item.label}>
                <title>{lbl}</title>
                <rect x={x - barW / 2} y={y} width={barW} height={bh} fill="#6b7280" opacity={0.75} rx={3} />
                <text x={x} y={y - 4} textAnchor="middle" fontSize={8} fill="#6b7280" fontWeight="700">{item.hours.toFixed(1)}h</text>
    ...
    <div style={card}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "0.75rem", flexWrap: "wrap", gap: "0.5rem" }}>
        ...
        <div style={{ display: "flex", gap: "1.25rem", fontSize: "0.75rem", color: "var(--color-text-secondary)" }}>
          <span style={{ display: "flex", alignItems: "center", gap: "0.3rem" }}><svg width="18" height="8"><rect x="0" y="2" width="18" height="6" rx="2" fill="#3b82f640" /></svg>{l ? "Horas" : "Hours"}</span>
          <span style={{ display: "flex", alignItems: "center", gap: "0.3rem" }}><svg width="18" height="8"><path d="M0,4 L18,4" stroke="#f59e0b" strokeWidth="2" strokeDasharray="4,2" /></svg>{l ? "Cant. WR" : "WR Count"}</span>
          <span style={{ display: "flex", alignItems: "center", gap: "0.3rem" }}><svg width="18" height="8"><path d="M0,4 L18,4" stroke="#10b981" strokeWidth="1.5" strokeDasharray="3,3" /></svg>{l ? "Promedio hrs" : "Avg hours"}</span>
        </div>
      </div>
      {hovItem && (
      ...
      <svg width="100%" viewBox={`0 0 ${w} ${h}`} style={{ overflow: "visible" }}>
        <defs><linearGradient id="gradH" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#3b82f6" stopOpacity="0.35" /><stop offset="100%" stopColor="#3b82f6" stopOpacity="0.03" /></linearGradient></defs>
        {[0.25, 0.5, 0.75, 1].map((pct) => {
          const y = padT + chartH - pct * chartH;
          return <g key={pct}><line x1={padL} x2={padL + chartW} y1={y} y2={y} stroke="var(--color-border)" strokeWidth={1} /><text x={padL - 4} y={y + 3} textAnchor="end" fontSize={8} fill="var(--color-text-secondary)">{(maxH * pct).toFixed(0)}</text></g>;
        })}
        ...
        {byDay.map((d, i) => <circle key={i} cx={toX(i)} cy={toYC(d.count)} r={hovered === i ? 4 : 2.5} fill="#f59e0b" opacity={hovered === i ? 1 : 0.7} />)}
        ...
        {(() => {
          ...
          return <g><line x1={px} x2={px} y1={py - 2} y2={py - 16} stroke="#ef4444" strokeWidth={1} strokeDasharray="2,2" /><rect x={px - 22} y={py - 30} width={44} height={14} rx={3} fill="#ef444420" stroke="#ef4444" strokeWidth={0.8} /><text x={px} y={py - 20} textAnchor="middle" fontSize={8} fill="#ef4444" fontWeight="700">{l ? "PICO" : "PEAK"} {byDay[peakHi].hours.toFixed(1)}h</text></g>;
        })()}
        {byDay.map((_, i) => <rect key={i} x={toX(i) - (chartW / byDay.length) / 2} y={padT} width={chartW / byDay.length} height={chartH} fill="transparent" onMouseEnter={() => setHovered(i)} onMouseLeave={() => setHovered(null)} />)}
        <line x1={padL} x2={padL} y1={padT} y2={padT + chartH} stroke="var(--color-border)" strokeWidth={1} />
        <line x1={padL} x2={padL + chartW} y1={padT + chartH} y2={padT + chartH} stroke="var(--color-border)" strokeWidth={1} />
        {byDay.map((d, i) => i % step === 0 ? <text key={i} x={toX(i)} y={padT + chartH + 14} textAnchor="middle" fontSize={8} fill="var(--color-text-secondary)">{d.date.slice(5)}</text> : null)}
      </svg>
      <div style={{ display: "flex", gap: "1.5rem", marginTop: "0.5rem", fontSize: "0.75rem", color: "var(--color-text-secondary)", borderTop: "1px solid var(--color-border)", paddingTop: "0.5rem" }}>
        ...
        <span>Total WR: <strong style={{ color: "var(--color-text-primary)" }}>{byDay.reduce((s, d) => s + d.count, 0)}</strong></span>
      </div>
    </div>
  );
}
...
export default function WorkRequestsPage() {
  ...
  useEffect(() => {
    ...
    MaintenanceService.getDowntimeByMonth(applied.start, applied.end)
      .then((res) => { if (!cancelled) setDowntimeMonth(res.data); })
      .catch((err: any) => { if (!cancelled) setDowntimeError(err?.message ?? (l ? "Error cargando paros" : "Failed to load downtime")); })
      .finally(() => { if (!cancelled) setDowntimeLoading(false); });
    return () => { cancelled = true; };
  }, [applied.start, applied.end, l]);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: "1rem" }}>
        <div><h1 style={{ fontSize: "1.375rem", fontWeight: 700, color: "var(--color-text-primary)", margin: 0 }}>{l ? "Mantenimiento — Work Requests" : "Work Requests"}</h1></div>
        <FilterBar draft={draft} setDraft={setDraft} onApply={apply} loading={loading || downtimeLoading} showShift={false} />
      </div>
      ...
      {loading ? (
        <div style={{ padding: "3rem", textAlign: "center", color: "var(--color-text-secondary)" }}>{l ? "Cargando Work Requests..." : "Loading Work Requests..."}</div>
      ) : data && (
        <>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem" }}>
            <TopFailureChart byFailure={data.by_failure} lang={lang} />
            {downtimeLoading ? (
              <div style={{ ...card, display: "grid", placeItems: "center", minHeight: 240, color: "var(--color-text-secondary)", fontSize: "0.8rem" }}>{l ? "Cargando paros..." : "Loading downtime..."}</div>
            ) : (
